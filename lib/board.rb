class Board
  ...
  def play_turn (player)
    puts "A toi de jouer #{player.name}. Fais un choix : ".yellow
    ...
    game_state
    if @victory == true
			puts "Bravo #{player.name}, tu as gagné.".green
  ...
  def show
    puts ""
    puts "   1   2  3".red
    puts " a #{@A1.content} | #{@A2.content} | #{@A3.content}".blue
    puts "   ---------"
    puts " b #{@B1.content} | #{@B2.content} | #{@B3.content}".blue
    puts "   ---------"
    puts " c #{@C1.content} | #{@C2.content} | #{@C3.content}".blue
    puts ""
  end
end